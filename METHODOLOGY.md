...
- **Renaiss OS Index API (`api.renaissos.com`)** — _integrated_ (partner-keyed). An **independent** cross-marketplace reference price per graded card. We cross-reference it against Renaiss's own FMV on the market page. Cards are matched **exact-only** (company + grade + set + number, since our listings carry no cert); anything not confidently matched is left unpriced rather than guessed. All figures are the Index's own valuations, attributed to "Renaiss OS Index" as their terms require.
...
4. **FMV is Renaiss's own valuation** — but now cross-referenced. The Renaiss OS Index (independent, cross-marketplace) is shown beside FMV on the market page; a large gap flags a possibly-stale FMV. The Index is **not** blended into the EV model (that stays a separate, labeled step) — it is displayed for contrast only.
...
Two honesty constraints are load-bearing here. First, both sides of the reconciliation rest on Renaiss's own FMV valuations (limitation #4), so it is an internal-consistency check, **not** an independent price check — the independent check lives on the market page, where the Renaiss OS Index is cross-referenced against FMV. Second, every frequency is a frequency in the *observed* feed, whose completeness is unknown (limitation #1); a gap between observed and claimed is a flag for scrutiny, **never proof of unfairness**, because feed curation explains a gap equally well. Packs below the 20-pull EV threshold show no distribution at all.
